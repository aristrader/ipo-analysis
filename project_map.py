...
CONTEXTS = {
    ...
    "news / announcement context feed (D1/D4)": [
        ...
        "docs/research/newsfeed/newsfeed_rnd_2026-06-09.md", "docs/research/newsfeed/newsfeed_opportunity_map.md",
    ],
    "backtest a strategy": [
    ...
    "the app / UI": ["app.py", "docs/research/app_phase2_design.md",
                     ...
                     "docs/research/newsfeed/newsfeed_opportunity_map.md",
    ...
    "test a hypothesis / research agent brief": [
        ...
        "docs/research/signals/tier1_wave1_verdicts.md",
    ],
    ...
    "testing / verification / the showdown": [
        ...
        "docs/research/archive/showdown_audit.md", "docs/research/archive/showdown_pipeline_diff.md",
        "docs/research/showdown_mutation.md",  # GENERATED by tools/mutation/run_mutations.py (not archived)
    ],
...
# --------------------------------------------------------------- FILE KINDS
# Machine-checkable file CLASSIFICATION (the DD-7 root-cause fix: verify.py knew
# existence but not KIND, so a GENERATED file rotting in archive/ while its generator
# wrote elsewhere was invisible — the showdown_mutation / DD-1/2/3 mistake-class).
#
# Each entry: path -> (KIND, generator_or_None). KINDS:
#   GENERATED        deterministic script/pipeline output (md/csv/html). generator REQUIRED.
#   GENERATED_INDEX  generated navigation index (MAP.md). generator REQUIRED.
#   CANONICAL        a hand-maintained authority (the brain / registries / schema).
#   HISTORICAL       append-only log; legitimately names old paths — exempt live-pointer rules.
#   ARCHIVED         superseded; must NOT be cited as a live source by a CANONICAL file.
# Anything under archive/ defaults to ARCHIVED (see file_kind()); only list the few
# files in the dangerous classes that need an explicit rule (~16, by design).
FILE_KINDS = {
    # GENERATED (rule A: must live at the generator's write-path, NOT only in archive/)
    "MAP.md":                                            ("GENERATED_INDEX", "verify.py"),
    ...
    # CANONICAL (rule B: must not be superseded-by-archive; counts live in MAP.md not here)
    "CLAUDE.md":                       ("CANONICAL", None),
    "project_map.py":                  ("CANONICAL", None),
    "rules/index.md":                  ("CANONICAL", None),
    "docs/schema.md":                  ("CANONICAL", None),
    "docs/tracker/STATUS.md":          ("CANONICAL", None),
    "docs/tracker/improvement_backlog.md": ("CANONICAL", None),
    "README.md":                       ("CANONICAL", None),
    # HISTORICAL (append-only logs; exempt from live-pointer enforcement)
    "docs/tracker/task_log.md":        ("HISTORICAL", None),
}

# The hand-maintained docs whose count tokens / archive citations are guarded.
GUARDED_DOCS = ["CLAUDE.md", "README.md", "docs/tracker/STATUS.md"]


def file_kind(path):
    """Return (KIND, generator) for a path. archive/ defaults to ARCHIVED; otherwise
    explicit FILE_KINDS entry, else (None, None)."""
    if path in FILE_KINDS:
        return FILE_KINDS[path]
    if "archive/" in path:
...
def render_map(n_tests=None):
    """Generate MAP.md from the structures above. Never hand-edit MAP.md.

    n_tests: exact pytest-collected count (verify.py full mode passes it; the fast
    hook does not, so the block shows tests='see verify.py' rather than a stale literal).
    This block is the SINGLE source of the volatile counts — hand-docs point HERE."""
    L = []
    ...
    tests_str = f"{n_tests} tests" if n_tests is not None else "tests: run `python verify.py`"
    L.append("## CANONICAL FACTS (generated — do not hand-edit; hand-docs must point here)")
    L.append(f"> {INVARIANTS['n_findings']} findings · {tests_str} · "
             f"{INVARIANTS['substrate_rows']} substrate rows · AS_OF {INVARIANTS['as_of_date']}")
    L.append("")
