...
- **Layer 3 — analysis.** ✅ BUILT (2026-05-31), reviewed + remediated. Engine in `layer3/` (UI-agnostic).
  **Part A** (descriptive report): `layer3/spine.py` (method spine) + findings in `layer3/findings/` → `run_layer3_report.py` → `report/layer3_partA.html` (**29 findings**, **207 tests**).
  ...
  **Part C** (backtester vs do-nothing): `layer3/backtest/` → `run_backtest.py`. **Cross-regime validation:**
  ...
  data_informed`). Tests: **207 total** — `tests/layer3/` (94, incl. 5-traps) + `tests/pipeline/` (37) +
...
- **`verify.py`** = the checkpoint: asserts every mapped path exists, the DAG is consistent, and invariants
  (29 findings / 207 tests / 2296 rows / AS_OF_DATE / data-vs-backup) hold; regenerates MAP.md. It runs
