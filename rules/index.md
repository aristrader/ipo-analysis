...
| wipeout-safety (own flags → return) | **QUALIFIES for the score — promotion RECOMMENDED, pending user confirm** | OOS with/without test: folding it in IMPROVES the lift in 14/18 cells — ALL THREE 3y-pooled cutoffs (+6 to +22pp; 2019: 55.1→76.8pp), MB-3y neutral, mixed only at 1y (no-edge horizon). Passes 'evolve-only-if-robust' at 3y. Held out of the production score (changes the headline number + 1y mixed) → recommend folding into `data_informed`; see NEEDS_YOUR_INPUT.md. Standalone gauge stays regardless. |
